...
import com.example.drivesenseapp.R
...
class LoggingAction(private val context: Context) {
    ...
    companion object {
        private const val NOTIFICATION_CHANNEL_ID = "drive_reminders"
        private const val NOTIFICATION_CHANNEL_NAME = "Drive Reminders"
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val channel = NotificationChannel(
                ...
                description = "Notifikacije za podsetke iz Drive Sense aplikacije"
            }
    ...
    fun createPostDriveReminder(text: String): Boolean {
        return try {
            ...
            android.util.Log.d("DriveReminders", "💾 Reminder saved: $text (will show on stop=true)")
    ...
    private fun showReminderNotification(reminder: Reminder) {
        try {
            ...
            try {
                ...
                android.util.Log.d("DriveReminders", "🔔 Notification shown: ${reminder.text}")
            } catch (e: SecurityException) {
                android.util.Log.e("DriveReminders", "❌ Permission denied for notifications: ${e.message}")
                Toast.makeText(context, "Enable notifications in settings", Toast.LENGTH_SHORT).show()
            }
        } catch (e: Exception) {
            android.util.Log.e("DriveReminders", "❌ Error showing notification: ${e.message}")
        }
    }
    ...
    fun showActiveRemindersNotifications() {
        android.util.Log.d("DriveReminders", "\n🛑 === STOP DETECTED - CHECKING REMINDERS ===")
        
        val activeReminders = getActiveReminders()
        
        android.util.Log.d("DriveReminders", "📋 Active reminders count: ${activeReminders.size}")
        
        if (activeReminders.isEmpty()) {
            android.util.Log.d("DriveReminders", "📤 No active reminders to show")
            return
        }
        
        android.util.Log.d("DriveReminders", "🔔 Showing ${activeReminders.size} active reminders as notifications:")
        
        // Prikaži sve reminders kao notifikacije
        activeReminders.forEachIndexed { index, reminder ->
            android.util.Log.d("DriveReminders", "  ${index + 1}. ${reminder.text}")
            showReminderNotification(reminder)
        }
        
        // Obriši sve reminders nakon prikaza
        android.util.Log.d("DriveReminders", "🗑️ Clearing all reminders from storage...")
        clearAllReminders()
        
        // Proveri da li su obrisani
        val remainingCount = getReminders().size
        android.util.Log.d("DriveReminders", "✅ Verification - Remaining reminders: $remainingCount")
        android.util.Log.d("DriveReminders", "========================================\n")
    }
    ...
    fun playSoftChimeForAggressivePattern() {
        try {
            ...
            try {
                ...
                if (ringtone != null) {
                    ringtone.play()
                    android.util.Log.d("DriveChime", "🔔 Chime pokrenut (RingtoneManager)")
                } else {
                    throw Exception("Ringtone je null")
                }
            } catch (e: Exception) {
                android.util.Log.d("DriveChime", "⚠️ RingtoneManager failed: ${e.message}, pokušavam ToneGenerator")
                ...
                try {
                    ...
                    android.util.Log.d("DriveChime", "🔔 Chime pokrenut (ToneGenerator)")
                    android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
                        toneGen.release()
                    }, 500)
                } catch (ex: Exception) {
                    android.util.Log.e("DriveChime", "❌ ToneGenerator failed: ${ex.message}")
            ...
            Toast.makeText(
                context,
                "🔔 Chime test - proveri Logcat sa tagom 'DriveChime'",
                Toast.LENGTH_SHORT
            ).show()
            
        } catch (e: Exception) {
            android.util.Log.e("DriveChime", "❌ Glavna greška: ${e.message}")
    ...
    fun printAllReminders() {
        ...
        if (reminders.isEmpty()) {
            android.util.Log.d("DriveReminders", "📋 REMINDERI: Nema sačuvanih remindera.")
            Toast.makeText(context, "Nema sačuvanih remindera.", Toast.LENGTH_SHORT).show()
            return
        }
        
        android.util.Log.d("DriveReminders", "")
        android.util.Log.d("DriveReminders", "═".repeat(60))
        android.util.Log.d("DriveReminders", "📋 SVI REMINDERI (${reminders.size}):")
        android.util.Log.d("DriveReminders", "═".repeat(60))
        ...
        reminders.forEachIndexed { index, reminder ->
            ...
            android.util.Log.d("DriveReminders", "")
            android.util.Log.d("DriveReminders", "${index + 1}. $status")
            android.util.Log.d("DriveReminders", "   Text: ${reminder.text}")
            android.util.Log.d("DriveReminders", "   Kreiran: $date")
            android.util.Log.d("DriveReminders", "   ID: ${reminder.id}")
        }
    ...
    fun clearAllReminders() {
        ...
        android.util.Log.d("DriveReminders", "🗑️ Svi reminderi ($count) su obrisani")
